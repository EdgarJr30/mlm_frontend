        ...
        <section className="flex-1 overflow-x-auto px-4 md:px-6 lg:px-8 pt-4 pb-8">
          {loading ? (
            ...
            <div className="space-y-6 max-w-4xl mx-auto">
              {tickets.map((ticket) => (
                <div key={ticket.id} className="border border-gray-200 bg-white shadow-sm rounded-lg overflow-hidden">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between p-4 border-b border-gray-200">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        Creado el{" "}
                        {ticket.created_at
                          ? new Date(ticket.created_at as unknown as string).toLocaleDateString("es-DO")
                          : "—"}
                      </p>
                    </div>
                    <div className="mt-2 md:mt-0 flex gap-2">
                      <span className="px-2 py-0.5 rounded text-xs font-medium border">{ticket.priority}</span>
                      <span className="px-2 py-0.5 rounded text-xs font-medium border">{ticket.status}</span>
                      {ticket.is_urgent && (
                        <span className="px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-700">Urgente</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="p-4 space-y-2 text-sm text-gray-700">
                    ...
                    <p><strong>Teléfono:</strong> {ticket.phone || "No especificado"}</p>
                    <p><strong>Fecha incidente:</strong> {ticket.incident_date}</p>
                    {ticket.deadline_date && <p><strong>Fecha límite:</strong> {ticket.deadline_date}</p>}
                    {ticket.comments && <p><strong>Comentarios:</strong> {ticket.comments}</p>}
