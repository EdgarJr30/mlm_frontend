...
export default function InboxBoard({ searchTerm, selectedLocation }: Props) {
  ...
  useLayoutEffect(() => {
    const isIndeterminate =
      selectedTicket.length > 0 && selectedTicket.length < tickets.length;
    ...
    try {
      ...
      if (isSearching) {
        setFilteredTickets((prev) =>
          prev.filter((ticket) => !ids.includes(ticket.id))
        );
      } else {
  ...
  useEffect(() => {
    if (isSearching) {
      ...
      setIsLoading(true);
      getFilteredTickets(searchTerm, selectedLocation, false).then(
        (results) => {
      ...
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto">
          <p className="mt-2 text-sm text-gray-700">
            Tickets pendientes de aprobación — seleccione uno o varios para
            aceptar
          </p>
                    ...
                    ticketsToShow.map((ticket) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-4 text-sm whitespace-nowrap text-gray-500">
                          ...
                          {ticket.image &&
                            (() => {
                              const imagePaths = getTicketImagePaths(
                                ticket.image
                              );
