...
export default function KanbanColumn({
   ...
   useEffect(() => {
    ...
    const loadMoreTickets = useCallback(async () => {
        ...
        console.log(`📤 Solicitando página ${page + 1} de "${status}"...`);

        setIsLoading(true);
        const newTickets = await getTicketsByStatusPaginated(status, page, pageSize);

        setTickets((prev) => {
            const merged = [...prev, ...newTickets];
            const unique = Array.from(new Map(merged.map(t => [t.id, t])).values());
            console.log(`✅ Total acumulado en "${status}": ${unique.length}`);
            return unique;
        });

        if (newTickets.length < pageSize) {
            console.log(`🛑 No hay más tickets para "${status}".`);
            setHasMore(false);
        }

        setPage((prev) => prev + 1);
        setIsLoading(false);
    }, [isLoading, hasMore, status, page, pageSize]);
