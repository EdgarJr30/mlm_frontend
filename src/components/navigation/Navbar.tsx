    ...
    <Disclosure as="nav" className="bg-white shadow-sm">
      <div className="mx-auto max-w-7xl px-2 sm:px-4 lg:px-8">
        <div className="flex h-16 justify-between items-center relative">
          ...
          <div className="flex items-center md:hidden relative" ref={dropdownRef}>
            <button
            ...
            {showDropdown && (
              <div className="absolute top-12 right-0 z-20 bg-white rounded shadow-lg w-48 border border-gray-200 animate-fade-in-down">
                <button
                  className={`w-full px-4 py-2 text-left hover:bg-indigo-50 ${selectedLocation === "" ? "font-semibold text-indigo-600" : "text-gray-700"}`}
                  onClick={() => {
                    onFilterLocation("");
                    setShowDropdown(false);
                  }}
                ...
                {LOCATIONS.map(loc => (
                  <button
                    key={loc}
                    className={`w-full px-4 py-2 text-left hover:bg-indigo-50 ${selectedLocation === loc ? "font-semibold text-indigo-600" : "text-gray-700"}`}
                    onClick={() => {
